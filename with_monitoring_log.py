        ...
        with SB(
            ...
            for theater_event in theater_events:
                ...
                if venue_url not in shows_by_venue_url:
                    if (
                        self.local_test
                        and getattr(self, "show_count", None) is not None
                        and len(shows_by_venue_url) >= self.show_count
                    ):
                        # Skip introducing new shows once local limit is hit
                        continue
                    shows_by_venue_url[venue_url] = self._new_show_record(theater_event)

                shows_by_venue_url[venue_url] = shows_by_venue_url.get(venue_url)
                if shows_by_venue_url[venue_url]:
            ...
            if self.local_test:
                self.custom_logger.info(
                    f"Local test mode active — processing {total_shows} show(s)"
                )
